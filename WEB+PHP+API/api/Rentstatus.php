...
$sql = "SELECT * FROM rentlist  LEFT JOIN items ON rentlist.item_name=items.item_name LEFT JOIN rentFormat ON rentlist.rentFormat= rentFormat.id LEFT JOIN statusR ON rentlist.status= statusR.id LEFT JOIN freeshow ON rentlist.item_name= freeshow.item_name WHERE username='$username' and status='1' or status='2' or status='3' GROUP BY rentlist.id DESC ;";
